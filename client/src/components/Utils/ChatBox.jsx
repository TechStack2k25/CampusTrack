...
export default function ChatBox({ user, setUser }) {
  ...
  const sendTheMessage = async () => {
    try {
      setIsSending(true);
      const res=await messageService.sendMessage({ _id: user?._id, text:text?.trim() });
      if(res) setText('');
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="w-80 h-96 border bg-white dark:bg-gray-800 shadow-lg p-4 flex flex-col rounded-lg">
      ...
      <div ref={messageRef} className="flex-1 flex flex-col-reverse p-2 overflow-y-auto hide-scrollbar">
