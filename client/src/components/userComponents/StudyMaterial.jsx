...
function StudyMaterial() {
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true);
      const res = await storeService.addMaterial({
        file: selectedFile,
        ...otherData,
        _id: courseId,
      });
        ...
        <div className='mt-4'>
          {materials.length > 0 ? (
            materials.map((material, index) => (
              <div
                key={material?._id}
                className='flex justify-between items-center bg-gray-200 dark:bg-gray-700 p-3 rounded-lg mb-2'
              >
      ...
      {showModal && (
        <div className='fixed z-30 inset-0 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='bg-white dark:bg-gray-800 border p-6 rounded-lg shadow-lg w-96 text-center'>
            ...
            <input
              type='file'
              className='block w-full p-2 border rounded-lg dark:text-gray-300'
