...
const Attendance = () => {
  ...
  const removeStudent = async (student,message) => {
    try {
      ...
      const res = await courseService.removeStudent_s({ _id: courseDetails._id, student_id: student._id, message });
      if (res) {
  ...
  return (
    <div className="flex-1 p-6 min-h-screen">
      ...
      >
        ...
        >
          {loading ? <span className="animate-spin "><FaSpinner /></span> : "Load Students"}
        </button>
      </form>
      ...
      {studentsData.length > 0 && (
            ...
            <div>
              {studentsData.map((student) => {
                const attendanceRate = student?.attendance
                  ? Math.round((student?.attendance / courseDetails?.total_classes) * 100)
                  : 0;
                return (
                  <div
                    key={student._id}
        ...
        hover:bg-green-400 hover:border-green-500 active:scale-95`}
                    ...
                    <p className={`font-medium flex items-center justify-center gap-2  ${attendanceRate >= 75 ? "text-green-500 font-semibold" : "text-red-500 font-bold"
                      }`}><span className="truncate"> Attended:</span> {student?.attendance || 0}</p>
                    <button
